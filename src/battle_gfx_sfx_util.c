...
static void BattleLoadMonSpriteGfx(struct Pokemon *mon, u32 battlerId, bool32 opponent)
{
    ...
    LoadPalette(decompressionBuffer, paletteOffset, PLTT_SIZE_4BPP);
    LoadPalette(decompressionBuffer, BG_PLTT_ID(8) + BG_PLTT_ID(battlerId), PLTT_SIZE_4BPP);
    ...
    if (state != 0)
    {
        ...
        else // IsTripleBattle()
        {
            if (state == 2)
            {
                ...
                {
                case 0: LoadCompressedSpriteSheet(&sSpriteSheet_SinglesPlayerHealthbox); break;
                case 1: LoadCompressedSpriteSheet(&sSpriteSheets_DoublesPlayerHealthbox[0]); break;
                case 2: LoadCompressedSpriteSheet(&sSpriteSheets_TriplesPlayerHealthbox[0]); break;
                }
            }
            else if (state == 3)
